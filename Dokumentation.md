...
## 0. Update: Total Alchemy & Isolation (2026-03-03)

Die finale Ausbaustufe ergaenzt vier produktive Schutz- und Lernpfade:

- Externe Rohdaten werden ueber `AlchemyIngestor` in `aether_events` und `alchemy_ingest` eingespeist.
- Jeder Ingest erzeugt eine Torch-basierte `fold_signature`, damit neue Signale direkt im holographischen Feld interferieren koennen.
- Das `PythonInterpreterTool` fuehrt Analysecode in einem separaten Prozess aus; der Core erhaelt nur das serialisierte Ergebnis.
- `LineageAuditor` bewertet Kind-Journale unter `DEMO/lineage/` kausal und waehlt daraus einen Betriebsmodus fuer transgeneratives Lernen.

Neue harte Safety-Invarianten:

- Ein toolbasiertes Proposal wird nur akzeptiert, wenn zwei aufeinanderfolgende Sandbox-Replays identisch sind.
- Der `AtherCoreKnot` (`Sicherheit`, `Reaktion`, `Analyse`, `Heilung`) darf durch Tools weder geloescht noch abgeschwaecht werden.
- Signierte Audit-Eintraege enthalten weiterhin HMAC-basierte Journal-Ketten; jetzt mit Determinismus- und Knotenschutz als Blockgruende.

## 0.1 Update: Realdaten-Ingest + Future Projection (2026-03-05)

- Finance-Ingest verwendet jetzt `yahoo -> stooq` als Fallback-Reihenfolge.
- Das minimale Auditintervall im Daemon wurde auf `10s` reduziert.
- Die Zukunftsprojektion fuehrt explizite Datenqualitaetsfelder:
  - `has_supervised_fit`, `events_missing_for_supervised_fit`,
  - `signal_mode_counts`, `market_signal_ratio`, `proxy_signal_ratio`.
- Beweisstatus bleibt bewusst konservativ, wenn Realdatenabdeckung zu gering ist.
...
- externe Daten-Alchemie fuer fremde, unstrukturierte Umweltdaten,
...
- transgeneratives Lernen ueber historische Integritaetsprofile aus Kind-Journalen.
